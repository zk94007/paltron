...
class ProcessJumplinks extends \ProcessWire\Process
{
  ...
  public function __construct()
  {
    ...
    // Get the module info
    $this->moduleInfo = \ProcessWire\wire('modules')->getModuleInfo($this, ['verbose' => true]);
   ...
   * @hook \ProcessWire\ProcessPageView::pageNotFound to scanAndRedirect
   * @return void
   */
    ...
    while ($this->_schemaVersion < self::SCHEMA_VERSION) {
      ...
      switch (true) {
        case ($memoryVersion <= 4):
          ...
          break;
        default:
          throw new \ProcessWire\WireException("[Jumplinks] Unrecognized database schema version: {$memoryVersion}");
      }
      if ($statement && $this->database->exec($statement) !== false) {
        ...
        throw new \ProcessWire\WireException("[Jumplinks] Couldn't update database schema to version {$memoryVersion}");
      }
    }
  }
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessJumplinks' . '/Classes/Blueprint.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
  ...
  /**
   * The fun part.
   * @caller Hook: \ProcessWire\ProcessPageView::pageNotFound
   * @return void
   */
  ...
  public function ___execute()
  {
    ...
    $tabContainer = new \ProcessWire\InputfieldWrapper();

    // Add the Jumplinks tab
    $jumplinksTab = new \ProcessWire\InputfieldWrapper();
    ...
    while ($jumplink = $jumplinks->fetchObject()) {
      ...
      $relativeStartTime = str_replace('Never', '', \ProcessWire\wirerelativetimestr($jumplink->date_start, true));
      $relativeEndTime = str_replace('Never', '', \ProcessWire\wirerelativetimestr($jumplink->date_end, true));
      ...
      $relativeLastHit = \ProcessWire\wirerelativetimestr($jumplink->last_hit, true);
    ...
    $mappingCollectionsTab = new \ProcessWire\InputfieldWrapper();
    ...
    while ($mappingCollection = $mappingCollections->fetchObject()) {
      ...
      $created = \ProcessWire\wirerelativetimestr($mappingCollection->created_at) . sprintf($this->_(' by %s'), $userCreated);
      $updated = \ProcessWire\wirerelativetimestr($mappingCollection->updated_at) . sprintf($this->_(' by %s'), $userUpdated);
    ...
    // Add Import tab
    $importTab = new \ProcessWire\InputfieldWrapper();
    ...
    if ($this->enable404Monitor) {
      // Add 404 Monitor tab.
      $notFoundMonitorTab = new \ProcessWire\InputfieldWrapper();
      ...
      // Get the UA parser
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessJumplinks' . '/Classes/ParseUserAgent.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
  ...
  public function ___executeEntity()
  {
    ...
    if (isset($destinationUriUrl) && $page = $this->pages->get((int) str_replace('page:', '', $destinationUriUrl))) {
      ...
      if ($isPage) {
        ...
        $destinationPathField->collapsed = \ProcessWire\Inputfield::collapsedYes;
        $destinationSelectorsFieldset->collapsed = \ProcessWire\Inputfield::collapsedNo;
      } else {
        $destinationSelectorsFieldset->collapsed = \ProcessWire\Inputfield::collapsedYes;
        $destinationPathField->collapsed = \ProcessWire\Inputfield::collapsedBlank;
      }
    } else {
      $destinationPageAutoField->collapsed = \ProcessWire\Inputfield::collapsedYes;
    }
    ...
    if ($editingId > 0) {
      ...
      $userUrl = \ProcessWire\wire('config')->urls->admin . 'access/users/edit/?id=';
      $relativeTimes = [
        'created' => \ProcessWire\wirerelativetimestr($createdAt),
        'updated' => \ProcessWire\wirerelativetimestr($updatedAt),
      ];
      $lastHitFormatted = $this->_("This jumplink hasn't been hit yet.");
      if (strtotime($lastHit) > $this->lowestDate) {
        $lastHitFormatted = sprintf($this->_('Last hit on: %s (%s)'), $lastHit, \ProcessWire\wirerelativetimestr($lastHit));
      }
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessJumplinks' . '/Classes/LeagueCsv/autoload.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
