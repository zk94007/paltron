...
{metadocument}
	<function>
		...
		<argument>
			...
			<documentation>
				<purpose>Not yet in use. It is meant to provide context
					information to interact with the end \ProcessWire\User.</purpose>
			</documentation>
		</argument>
		<do>
{/metadocument}
		...
		for($m=0;$m<count($mechanisms);$m++)
		{
			...
			if(IsSet($this->drivers[$mechanism]))
			{
				if(!class_exists($this->drivers[$mechanism][0]))
					require(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/WireMailSmtp/smtp_classes/sasl.php')."/".$this->drivers[$mechanism][1]);
