...
********************************************************************************/
 ...
 require_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/WireMailSmtp/WireMailSmtpAdaptor.php') . '/smtp_classes/email_message.php' ,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 require_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/WireMailSmtp/WireMailSmtpAdaptor.php') . '/smtp_classes/smtp_message.php' ,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 require_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/WireMailSmtp/WireMailSmtpAdaptor.php') . '/smtp_classes/smtp.php' ,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 require_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/WireMailSmtp/WireMailSmtpAdaptor.php') . '/smtp_classes/sasl.php' ,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
...
class hnsmtp {
	...
	private function logError($msg) {
		if(!isset($this->module)) $this->module = \ProcessWire\wire('modules')->get('WireMailSmtp');
	...
	public function isValidEmailadress($email) {
		...
		$clean = \ProcessWire\wire('sanitizer')->email($email);
		if($email != $clean) throw new \ProcessWire\WireException("Invalid email address");
		return true;
	}
	...
	private function bundleEmailAndName($email, $name) {
		...
		$name = \ProcessWire\wire('sanitizer')->emailHeader($name);
